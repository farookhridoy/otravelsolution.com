<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Booking List</h1>
...
<div class="bg-white rounded-md shadow overflow-x-auto">
   <table class="w-full whitespace-nowrap">
      <tr class="text-left font-bold">
         ...
         <th class="px-6 pt-6 pb-4">Pickup Date & Time</th>
      ...
      <tr v-for="values in booking_data.data" :key="values.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
      ...
      <tr v-if="booking_data.data.length === 0">
         <td class="border-t px-6 py-4" colspan="12">No data found.</td>
      </tr>
   </table>
</div>
<pagination class="mt-6" :links="booking_data.links" />
</div>
</template>
   ...
   watch: {
      form: {
         handler: throttle(function() {
            ...
            this.$inertia.replace(this.route('booking_data', Object.keys(query).length ? query : { remember: 'forget' }))
         }, 150),
         deep: true,
      },
   }
